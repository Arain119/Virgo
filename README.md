...
Virgo Trader 将交易过程抽象为 Gymnasium 环境：智能体在每个时间步基于历史窗口特征（技术指标、时间特征、可选情绪特征）与账户状态，输出连续动作并获得奖励。训练使用 Stable-Baselines3 的 PPO（Actor-Critic）来优化策略。

### 状态、动作与交易成本

- 状态（Observation）：以 `window_size` 为长度的时间序列矩阵，包含特征工程后的市场特征，并拼接账户特征（仓位比例、现金比例等）与动作历史。
- 动作（Action）：
  - 单标的环境：连续的目标仓位比例 `0~1`。
  - 多标的环境：连续的组合权重向量 `0~1`（必要时会归一化使权重和不超过 1）。
- 成本：在环境中显式模拟手续费 `commission_rate` 与滑点 `slippage`，并记录交易明细与资产曲线。

### 复合奖励函数（风险收益权衡）

奖励并非只看单步收益，而是将多种信号组合为一个标量（可在代码中配置权重），包括：

- Differential Sharpe Ratio（DSR）：鼓励更稳定的风险调整后收益。
- 简单收益与相对基准的超额收益。
- 回撤惩罚、相关性惩罚（用于约束策略与标的收益的相关性行为）、以及“长时间空仓”的惩罚。

### 策略网络骨干（可选）

PPO 的策略网络提供多种特征提取骨干：

- `multiscale_cnn`：多尺度 1D CNN 提取短/中/长周期模式。
- `transformer`：基于自注意力的时序建模（含相对位置偏置）。
- `cross_attention`：融合 CNN 与 Transformer 信息的交叉注意力结构。

### 调参与验证（Optuna + 早停）

项目提供 Optuna 超参搜索与训练阶段的验证早停机制（时间序列切分），减少过拟合与无效计算。评估指标包括收益、夏普、回撤等常用统计量（具体以代码实现为准）。

### 情绪特征（可选）

情绪数据以 JSONL 为载体，先按“新闻发布时间”聚合到日级特征，再与交易日历对齐（包含周末/节假日向后映射到下一个交易日的处理），并生成多尺度 EMA/变化率等派生特征，供训练/回测按需加载。
